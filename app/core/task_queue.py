...
class TaskQueue:
    ...
    async def initialize(self) -> None:
        ...
        await self._connection.execute(text(f"LISTEN {self._task_channel}"))
        ...
        logger.info("TaskQueue initialized with channel %s", self._task_channel)
    ...
    async def _notification_listener(self) -> None:
        ...
        try:
            while True:
                try:
                    # Wait for notification
                    notification = await self._connection.connection.driver_connection.notifies()
                    if notification.channel == self._task_channel:
                        logger.debug("Got notification for new task: %s", notification.payload)
